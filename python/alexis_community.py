...
def modularity(partition, graph, gamma) :
    ...
    we implement the modularity Q = 1/(2W) \sigma_i,j (A_ij - gamma P_ij)*delta(C_i,C_j)
    """
    ...
    for node_i in graph:
        for node_j in graph:
            com_i = partition[node_i]
            com_j = partition[node_j]
            if (com_i == com_j):
                A_ij = 0.
                if graph.has_edge(node_i, node_j):
                    A_ij = graph.get_edge_data(node_i, node_j).get("weight", 1)
                k_i = graph.degree(node_i, weight = 'weight')
                k_j = graph.degree(node_j, weight = 'weight')
                P_ij = (k_i*k_j) / (2.*W)
                Q += (A_ij - gamma*P_ij)
            else:
                # we only sum over cases in which nodes i and j are classified in the same community
                pass 
...
def best_partition(graph, gamma, partition = None) :
    ...
    >>> plt.show()
    """
    dendo = generate_dendrogram(graph, gamma, partition)
    return partition_at_level(dendo, len(dendo) - 1 )


def generate_dendogram(graph, gamma, part_init = None) :
    """Deprecated, use generate_dendrogram"""
    return generate_dendrogram(graph, gamma, part_init)
    
    
def generate_dendrogram(graph, gamma, part_init = None) :
    ...
    mod = __modularity(status, gamma)
    status_list = list()
    __one_level(current_graph, status, gamma)
    new_mod = __modularity(status, gamma)
...
def __one_level(graph, status, gamma) :
    """Compute one level of communities
    """
    modif = True
    nb_pass_done = 0
    cur_mod = __modularity(status, gamma)
    new_mod = cur_mod
...
class Status :
    ...
    def copy(self) :
        ...
        new_status.graph_ref = self.graph_ref.copy()

    def init(self, graph, part = None) :
        """Initialize the status of a graph with every node in one community"""
        self.graph_ref = graph.copy()
        count = 0
        self.node2com = dict([])
        self.total_weight = 0
        self.degrees = dict([])
...
def __modularity(status, gamma) :
    """
    Compute the modularity of the partition of the graph faslty using status precomputed
    """

    W = status.graph_ref.size(weight='weight')
    Q = 0.
    for node_i in status.graph_ref:
        for node_j in status.graph_ref:
            com_i = status.node2com[node_i]
            com_j = status.node2com[node_j]
            if (com_i == com_j):
                A_ij = 0.
                if status.graph_ref.has_edge(node_i, node_j):
                    A_ij = status.graph_ref.get_edge_data(node_i, node_j).get("weight", 1)
                k_i = status.graph_ref.degree(node_i, weight = 'weight')
                k_j = status.graph_ref.degree(node_j, weight = 'weight')
                P_ij = (k_i*k_j) / (2.*W)
                Q += (A_ij - gamma*P_ij)
            else:
                # we only sum over cases in which nodes i and j are classified in the same community
