...
G = Graph.Read_GraphML("miserables.GraphML")    #igraph version of data for partitioning

K = nx.read_graphml("miserables.GraphML")       #networkx version of data for json-ing
...
#for res_par in [10.,5.,1.,0.5,0.1]:
    if (first):
        ...
        for i,n in enumerate(K.nodes()):
            #print n
            idx = G.vs.select(id=n)[0].index  #this code is specific to the karate club, to get everything in the right order
            #print idx
...
#print data['nodes']
with open('miserables_rb.js', 'w') as outfile:
